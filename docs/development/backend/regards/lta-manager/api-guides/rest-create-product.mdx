 ...
 * if you use a endpoint with **replace deactivated**, a new version is created (V2, V3...)
 * if you use the endpoint with **replace deactivated**, the previous version is replaced
...
On [AMQP interface](amqp-submit-product.md), there is an additional `owner` parameter. When you use the REST API, the
product owner is
