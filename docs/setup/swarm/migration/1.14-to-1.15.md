...
There are some steps remaining at the end of this guide related to RabbitMQ.
:::
...
:::info
`ADMIN_USER` must have access rights.

```bash
rabbitmqctl list_users
Listing users ...
user	tags
regards-ias-qua    [administrator]

rabbitmqctl list_user_permissions regards-ias-qua
Listing permissions for user "regards-ias-qua" ...
vhost configure write read
regards.multitenant.manager .* .* .*
```
