...
`sslmode` (either `require`or `prefer`). This is done by modifying the `admin.t_project_connection` table in the
instance (Append sslmode=require or sslmode=prefer to the project connection url)
