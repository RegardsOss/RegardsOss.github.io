...
=== How to generate specific documentation snippets?
...
TIP: *Optional field* has to be marked as optional using `FieldDescriptor#optional()` method and `FieldDescriptor#type("String")` (Look at <<json-field-types>> for available type).
...
To aggregate test snippets, create an API aggregator file in `src/main/asciidocs`.
