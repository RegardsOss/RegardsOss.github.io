...
* `service plugins`, that adds functionalities to a results or a selection of results ([more detail here](/frontend/plugins/plugin-services/)).
...
* lint plugin code
In order to use those commands, rs-frontend repository must have been installed (see [Frontend setup page](/frontend/setup/)), as it provides the core webpack DLLs, used to lower compilation duration and plugin bundle size. The plugin must also

## 1. Install plugin

Before running any other command, the one below should be performed in plugin main folder, to install it:
...
1. The plugin bundle must be available. Method depends on development phase (see sections hereafter)
